...
%old: matlabbatch{1}.spm.temporal.st.scans{1} = cellstr(spm_select('ExtFPList',func_dir,sprintf('^vol.*.nii'),Inf));
matlabbatch{1}.spm.temporal.st.scans{1} = cellstr(spm_select('ExtFPList', func_dir, '^sub-.*_bold\.nii$', Inf));
matlabbatch{1}.spm.temporal.st.nslices = 45; %40 = previous value
...
matlabbatch{1}.spm.temporal.st.so = 1:45; %40 = previous value
matlabbatch{1}.spm.temporal.st.refslice = 23; %20 = previous value
