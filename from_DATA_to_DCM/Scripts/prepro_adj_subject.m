function matlabbatch = prepro_adj_subject(dataDir, run)

%script for preprocessing fMRI data
...
%The fMRI scans were recorded in an interleaved manner
% The slice order is stored in a .json file
...
for i = 1:length(nii_files)
    ...
    % Extract parameters & obtain slice order
    ...
    matlabbatch{1}.spm.temporal.st.so = so(:); % (:)ensurs collumn vector
